...
;(function(global) {
  ...
  var root = global;
	...
	*/
	...
	if (!global.console)
		global.console = { time:function(){}, timeEnd:function(){}, group:function(){}, groupEnd:function(){}, log:function(){} };
			...
			_processOverlay = function(component, o) {
				...
				if (_ju.isArray(o)) {	// this is for the shorthand ["Arrow", { width:50 }] syntax
					...
					var type = o[0], 
						...
						p = global.jsPlumb.extend({component:component, _jsPlumb:component._jsPlumb.instance}, o[1]);
					...
					_newOverlay = new global.jsPlumb.Overlays[component._jsPlumb.instance.getRenderMode()][o]({component:component, _jsPlumb:component._jsPlumb.instance});
				} else {
					_newOverlay = o;
				}										
		...
		var jsPlumbInstance = global.jsPlumbInstance = function(_defaults) {
			...
			var _currentInstance = this,
	            ...
	            _info = function(el) {
	            	...
	            	return { el:_el, id:(global.jsPlumbUtil.isString(el) && _el === null) ? el : _getId(_el) };
	            };
...
// --------------------------- jsPlumbInstance public API ---------------------------------------------------------
		...
		*/
		...
		this.getDefaultEndpointType = function() { return global.jsPlumb.Endpoint; };		
		// gets the default connection type. used when subclassing.  see wiki.
		this.getDefaultConnectionType = function() { return global.jsPlumb.Connection; };
		/*
		...
		this.init = function() {
			...
			if (!initialized) {                
                ...
                _currentInstance.anchorManager = new global.jsPlumb.AnchorManager({jsPlumbInstance:_currentInstance});                
				...
				if (_ju.isArray(specimen[0]) || _ju.isString(specimen[0])) {
					...
					if (specimen.length == 2 && _ju.isObject(specimen[1])) {
						...
						if (_ju.isString(specimen[0])) {
							pp = global.jsPlumb.extend({elementId:elementId, jsPlumbInstance:_currentInstance}, specimen[1]);
							newAnchor = _a(specimen[0], pp);
						}
						// otherwise first arg is array, second is params. we treat as a dynamic anchor, which is fine
						// even if the first arg has only one entry. you could argue all anchors should be implicitly dynamic in fact.
						else {
							pp = global.jsPlumb.extend({elementId:elementId, jsPlumbInstance:_currentInstance, anchors:specimen[0]}, specimen[1]);
							newAnchor = new global.jsPlumb.DynamicAnchor(pp);
						}
					}
					else
						newAnchor = new global.jsPlumb.DynamicAnchor({anchors:specimen, selector:null, elementId:elementId, jsPlumbInstance:_currentInstance});
					...
					newAnchor.clone = function() { return new global.jsPlumb.Anchor(anchorParams); };						 					
				}
			}
		...
		this.makeAnchors = function(types, elementId, jsPlumbInstance) {
			...
			for ( var i = 0, ii = types.length; i < ii; i++) {
				if (typeof types[i] == "string")
					r.push(global.jsPlumb.Anchors[types[i]]({elementId:elementId, jsPlumbInstance:jsPlumbInstance}));
		...
		this.makeDynamicAnchor = function(anchors, anchorSelector) {
			return new global.jsPlumb.DynamicAnchor({anchors:anchors, selector:anchorSelector, elementId:null, jsPlumbInstance:_currentInstance});
		};
...
// --------------------- makeSource/makeTarget ---------------------------------------------- 
		...
		this.makeTarget = function(el, params, referenceParams) {
			...
			var p = global.jsPlumb.extend({_jsPlumb:this}, referenceParams);
		 global.jsPlumb.extend(p, params);
			...
			var maxConnections = p.maxConnections || 1,
				...
				_doOne = function(elInfo) {
					...
					var mouseDownListener = function(e) {
						...
						// will be located.
						var elxy = global.jsPlumbAdapter.getPositionOnElement(evt, _del, _zoom), pelxy = elxy;
...
global.jsPlumb = new global.jsPlumbInstance(); global.jsPlumb.getInstance = function(_defaults) { var j = new global.jsPlumbInstance(_defaults); j.init(); return j; };
})(typeof exports !== undefined ? exports : this);
/*
 * jsPlumb
 * 
...
;(function(global) {
    ...
    var _makeFloatingEndpoint = function(paintStyle, referenceAnchor, endpoint, referenceCanvas, sourceElement, _jsPlumb, _newEndpoint, scope) {
        var floatingAnchor = new global.jsPlumb.FloatingAnchor( { reference : referenceAnchor, referenceCanvas : referenceCanvas, jsPlumbInstance:_jsPlumb });
    ...
    var findConnectionIndex = function(conn, ep) {
        return global.jsPlumbUtil.findWithFunction(ep.connections, function(c) { return c.id == conn.id; });
    };
...
;(function(global) {
    ...
    global.jsPlumb.Connection = function(params) {
        var _newEndpoint = params.newEndpoint, _ju = global.jsPlumbUtil;
...
// --------------------------------------------------------------------------------------
            ...
            placeAnchors = function(elementId, _anchorLists) {		
                var cd = global.jsPlumbInstance.getCachedData(elementId), sS = cd.s, sO = cd.o,
        ...
        var _updateAnchorList = function(lists, theta, order, conn, aBoolean, otherElId, idx, reverse, edgeId, elId, connsToPaint, endpointsToPaint) {        
            ...
            if (listToRemoveFrom) {
                var rIdx = global.jsPlumbUtil.findWithFunction(listToRemoveFrom, function(e) { return e[4] == endpointId; });
                if (rIdx != -1) {
                    listToRemoveFrom.splice(rIdx, 1);
                    // get all connections from this list
                    for (i = 0; i < listToRemoveFrom.length; i++) {
                        global.jsPlumbUtil.addWithFunction(connsToPaint, listToRemoveFrom[i][1], function(c) { return c.id == listToRemoveFrom[i][1].id; });
                        global.jsPlumbUtil.addWithFunction(endpointsToPaint, listToRemoveFrom[i][1].endpoints[idx], function(e) { return e.id == listToRemoveFrom[i][1].endpoints[idx].id; });
                        global.jsPlumbUtil.addWithFunction(endpointsToPaint, listToRemoveFrom[i][1].endpoints[oIdx], function(e) { return e.id == listToRemoveFrom[i][1].endpoints[oIdx].id; });
                    }
                }
            }
            ...
            for (i = 0; i < listToAddTo.length; i++) {
                ...
                global.jsPlumbUtil.addWithFunction(connsToPaint, listToAddTo[i][1], function(c) { return c.id == listToAddTo[i][1].id; });                
                global.jsPlumbUtil.addWithFunction(endpointsToPaint, listToAddTo[i][1].endpoints[idx], function(e) { return e.id == listToAddTo[i][1].endpoints[idx].id; });
                global.jsPlumbUtil.addWithFunction(endpointsToPaint, listToAddTo[i][1].endpoints[oIdx], function(e) { return e.id == listToAddTo[i][1].endpoints[oIdx].id; });
            }
            if (exactIdx != -1) {
		...
		this.redraw = function(elementId, ui, timestamp, offsetToUI, clearEdits, doNotRecalcEndpoint) {
			...
			if (!global.jsPlumbInstance.isSuspendDrawing()) {
				...
				var myOffset = global.jsPlumbInstance.updateOffset( { elId : elementId, offset : ui, recalc : false, timestamp : timestamp }),
	                orientationCache = {};
	            ...
	            for (var i = 0; i < endpointConnections.length; i++) {
	                ...
	                if (sourceContinuous || targetContinuous) {
		                ...
		                if (elementId != targetId) global.jsPlumbInstance.updateOffset( { elId : targetId, timestamp : timestamp }); 
		                if (elementId != sourceId) global.jsPlumbInstance.updateOffset( { elId : sourceId, timestamp : timestamp }); 
		                ...
		                if (sourceContinuous) global.jsPlumbUtil.addWithFunction(anchorsToUpdate, sourceId, function(a) { return a === sourceId; });
		                if (targetContinuous) global.jsPlumbUtil.addWithFunction(anchorsToUpdate, targetId, function(a) { return a === targetId; });
		                global.jsPlumbUtil.addWithFunction(connectionsToPaint, conn, function(c) { return c.id == conn.id; });
		                if ((sourceContinuous && oIdx === 0) || (targetContinuous && oIdx === 1))
		                	global.jsPlumbUtil.addWithFunction(endpointsToPaint, conn.endpoints[oIdx], function(e) { return e.id == conn.endpoints[oIdx].id; });
		            }
	            }
				...
				for (i = 0; i < ep.length; i++) {
					if (ep[i].connections.length === 0 && ep[i].anchor.isContinuous) {
						...
						global.jsPlumbUtil.addWithFunction(anchorsToUpdate, elementId, function(a) { return a === elementId; });
					}
				}
				...
				for (i = 0; i < endpointConnections.length; i++) {
					...
					if (otherEndpoint.anchor.constructor == global.jsPlumb.DynamicAnchor) {			 							
	                    ...
	                    global.jsPlumbUtil.addWithFunction(connectionsToPaint, endpointConnections[i][0], function(c) { return c.id == endpointConnections[i][0].id; });
						// all the connections for the other endpoint now need to be repainted
						for (var k = 0; k < otherEndpoint.connections.length; k++) {
							if (otherEndpoint.connections[k] !== endpointConnections[i][0])							
	                            global.jsPlumbUtil.addWithFunction(connectionsToPaint, otherEndpoint.connections[k], function(c) { return c.id == otherEndpoint.connections[k].id; });
						}
...
// ********************************* OVERLAY DEFINITIONS ***********************************************************************    
 ...
 global.jsPlumb.Overlays.Arrow = function(params) {
		...
		params = params || {};
		var _ju = global.jsPlumbUtil, _jg = Biltong;
    ...
    global.jsPlumb.Overlays.PlainArrow = function(params) {
    	params = params || {};    	
    	var p = global.jsPlumb.extend(params, {foldback:1});
    ...
    global.jsPlumb.Overlays.Diamond = function(params) {
    	...
    	var l = params.length || 40,
    	    p = global.jsPlumb.extend(params, {length:l/2, foldback:2});
     global.jsPlumb.Overlays.Arrow.call(this, p);
    	this.type = "Diamond";
    };
